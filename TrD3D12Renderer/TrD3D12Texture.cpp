...
void TrD3D12Texture::Initialize2D(
    ID3D12Device* device,
    UINT64 width,
    UINT height,
    DXGI_FORMAT format,
...
void TrD3D12Texture::CreateRenderTargetView(
    ID3D12Device* device,
...
void TrD3D12Texture::CreateDepthStencilView(
    ID3D12Device* device,
    D3D12_CPU_DESCRIPTOR_HANDLE handle,
    DXGI_FORMAT viewFormat) const
{
...
void TrD3D12Texture::CreateShaderResourceView(
    ID3D12Device* device,
    D3D12_CPU_DESCRIPTOR_HANDLE handle,
    DXGI_FORMAT viewFormat) const
{
...
void TrD3D12Texture::CreateUnorderedAccessView(
    ID3D12Device* device,
    D3D12_CPU_DESCRIPTOR_HANDLE handle,
    DXGI_FORMAT viewFormat) const
{
...
bool TrD3D12Texture::Transition(
    ID3D12GraphicsCommandList* commandList,
    D3D12_RESOURCE_STATES newState)
{
    ValidateResource();
    const bool recorded = TrD3D12ResourceBarrier::Transition(
        commandList,
        mResource.Get(),
        mState,
        newState);
...
void TrD3D12Texture::UavBarrier(ID3D12GraphicsCommandList* commandList) const
{
    ValidateResource();
    TrD3D12ResourceBarrier::Uav(commandList, mResource.Get());
}

DXGI_FORMAT TrD3D12Texture::ResolveViewFormat(DXGI_FORMAT viewFormat) const
{
    return viewFormat == DXGI_FORMAT_UNKNOWN ? mDescription.Format : viewFormat;
}

void TrD3D12Texture::ValidateResource() const
{
