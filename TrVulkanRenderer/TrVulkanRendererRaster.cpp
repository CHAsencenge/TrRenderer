#include "TrVulkanRendererRaster.h"
...
TrVulkanRendererRaster::TrVulkanRendererRaster(uint32_t width, uint32_t height, const char* title) : TrVulkanRendererBase(width, height, title)
{
}

void TrVulkanRendererRaster::Run()
{
	OnInitWindow();
	OnInitVulkan();
	OnRender();
...
void TrVulkanRendererRaster::OnSetupImGui()
{
	// imgui
...
void TrVulkanRendererRaster::OnRenderImGui()
{
	ImGuiIO& io = ImGui::GetIO(); (void)io;
...
void TrVulkanRendererRaster::OnRender()
{
...
void TrVulkanRendererRaster::OnCleanup()
{
...
void TrVulkanRendererRaster::CreateInstance()
{
	VkApplicationInfo appInfo = {};
	appInfo.sType = VK_STRUCTURE_TYPE_APPLICATION_INFO;
	appInfo.pApplicationName = "TrVulkanRendererRaster";
...
void TrVulkanRendererRaster::LoadModel(const std::string& filename, uint32_t actorId, glm::mat4 transform)
{
	TrVulkanRendererBase::LoadModel(filename, actorId, transform);
	...
	CreateOrReCreateBuffer(objLoader.m_vertices.size() * sizeof(VertexObj), VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, alignedBufferSize, model.mVertexBuffer, *mem0);

	mDeviceMemoriesMap[std::string{"IntexBuffer"} + std::to_string(actorId)] = std::make_pair(&model.mVertexBuffer, mem0);
	CreateOrReCreateBuffer(objLoader.m_indices.size() * sizeof(VertexObj), VK_BUFFER_USAGE_INDEX_BUFFER_BIT, alignedBufferSize, model.mIndexBuffer, *mem1);
	
	mDeviceMemoriesMap[std::string{"MatColorBuffer"} + std::to_string(actorId)] = std::make_pair(&model.mVertexBuffer, mem0);
	CreateOrReCreateBuffer(objLoader.m_materials.size() * sizeof(VertexObj), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | flag, alignedBufferSize, model.mMatColorBuffer, *mem2);
...
void TrVulkanRendererRaster::SetupDebugMessenger()
{
	if(!mbEnableValidationLayers)
	{
		return;
...
VkResult TrVulkanRendererRaster::CreateDebugUtilsMessengerEXT(VkInstance instance,
...
void TrVulkanRendererRaster::DestroyDebugUtilsMessengerEXT(VkInstance instance, VkDebugUtilsMessengerEXT debugMessenger,
...
void TrVulkanRendererRaster::PickFirstValidPhysicalDevice(VkQueueFlagBits queueFlag)
{
...
void TrVulkanRendererRaster::PickHighestWeightScorePhysicalDevice(VkQueueFlagBits queueFlag)
{
	uint32_t deviceCount = 0;
	vkEnumeratePhysicalDevices(mInstance, &deviceCount, nullptr);
	if(deviceCount == 0)
...
bool TrVulkanRendererRaster::IsDeviceSuitable(VkPhysicalDevice device, VkQueueFlagBits queueFlag)
{
...
uint32_t TrVulkanRendererRaster::CalcDeviceSuitabilityScore(VkPhysicalDevice device, VkQueueFlagBits queueFlag)
{
...
void TrVulkanRendererRaster::CreateLogicalDevice()
{
...
TrVulkanQueueFamilyIndices TrVulkanRendererRaster::FindQueueFamilies(VkPhysicalDevice device, VkQueueFlagBits queueFlag)
{
...
void TrVulkanRendererRaster::CreateSurface()
{
...
TrVulkanSwapChainSupportDetails TrVulkanRendererRaster::QuerySwapChainSupport(VkPhysicalDevice device)
{
...
VkSurfaceFormatKHR TrVulkanRendererRaster::ChooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats)
{
...
// vertical retrace
// tearing
VkPresentModeKHR TrVulkanRendererRaster::ChooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes)
{
...
// resolution
VkExtent2D TrVulkanRendererRaster::ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities)
{
...
void TrVulkanRendererRaster::CreateSwapChain()
{
...
void TrVulkanRendererRaster::CreateSwapChainImageViews()
{
...
void TrVulkanRendererRaster::CleanupSwapChain()
{
...
void TrVulkanRendererRaster::CreateRenderPass()
{
...
VkShaderModule TrVulkanRendererRaster::CreateShaderModule(std::vector<char>& compiledCode)
{
...
void TrVulkanRendererRaster::CreateFrameBuffers()
{
...
void TrVulkanRendererRaster::CreateCommandPool()
{
...
void TrVulkanRendererRaster::CreateCommandBuffers()
{
...
void TrVulkanRendererRaster::RecordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex)
{
	OnRenderImGui();
...

	{
		...
		if (!bMinimized)
		{
			...
			if(imGuiDrawData->TotalVtxCount > 0)
			{
				...
				CreateOrReCreateBuffer(imGuiVertexSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, alignedVertexBufferSize, mImGuiVertexBuffers[mCurrentFrame], mImGuiVertexBufferMemories[mCurrentFrame], VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
				CreateOrReCreateBuffer(imGuiIndexSize, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, alignedIndexBufferSize, mImGuiIndexBuffers[mCurrentFrame], mImGuiIndexBufferMemories[mCurrentFrame], VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void TrVulkanRendererRaster::CopyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
...
void TrVulkanRendererRaster::CreateVertexBuffer()
{
	...
	VkDeviceSize alignedBufferSize = 0;
	CreateOrReCreateBuffer(bufferSize, usageStaging,  alignedBufferSize, stagingBuffer, stagingBufferMemory, propertyFlags);
	...
	VkDeviceSize alignedVertexBufferSize = 0;
	CreateOrReCreateBuffer(bufferSize, usageVertex,  alignedVertexBufferSize, mVertexBuffer, mVertexBufferMemory);
...
void TrVulkanRendererRaster::CreateIndexBuffer()
{
	VkBuffer stagingBuffer = VK_NULL_HANDLE;
	VkDeviceMemory stagingBufferMemory = VK_NULL_HANDLE;
	...
	VkDeviceSize alignedIndexBufferSize = 0;
	CreateOrReCreateBuffer(bufferSize, usageIndex,  alignedIndexBufferSize, mIndexBuffer, mIndexBufferMemory);
...
uint32_t TrVulkanRendererRaster::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags propertyFlags)
{
...
VkCommandBuffer TrVulkanRendererRaster::BeginSingleTimeCommands()
{
...
void TrVulkanRendererRaster::EndSingleTimeCommands(VkCommandBuffer commandBuffer)
{
	// end command buffer
...
void TrVulkanRendererRaster::CreateDepthResources()
{
...
VkFormat TrVulkanRendererRaster::FindSupportedFormat(const std::vector<VkFormat> candidates, VkImageTiling tiling,
...
void TrVulkanRendererRaster::DrawFrame()
{
...
void TrVulkanRendererRaster::UpdateUniformBuffer(uint32_t currentImage)
{
...
void TrVulkanRendererRaster::CreateSyncObjects()
{
...
void TrVulkanRendererRaster::CreateDescriptorSetLayout()
{
...
void TrVulkanRendererRaster::CreateDescriptorPool()
{
...
void TrVulkanRendererRaster::CreateDescriptorSets()
{
...
void TrVulkanRendererRaster::CreateTextureImage()
{
	// load texture file
	int texWidth, texHeight, texChannels;
	...
	VkDeviceMemory stagingBufferMemory = VK_NULL_HANDLE;
	VkDeviceSize alignedBufferSize = 0;
	CreateOrReCreateBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, alignedBufferSize, stagingBuffer, stagingBufferMemory, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void TrVulkanRendererRaster::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
...
void TrVulkanRendererRaster::CreateTextureSampler()
{
...
void TrVulkanRendererRaster::LoadModels(std::vector<std::string> filenames)
{
...
// populate io.Fonts
void TrVulkanRendererRaster::ImGuiCreateFontsTexture(VkCommandBuffer commandBuffer)
{
	...
	VkDeviceMemory stagingBufferMemory = VK_NULL_HANDLE;
	VkDeviceSize alignedBufferSize = 0;
	CreateOrReCreateBuffer(uploadSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, alignedBufferSize, stagingBuffer, stagingBufferMemory, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
...
void TrVulkanRendererRaster::ImGuiSetFontTexId()
{
	ImGuiIO& io = ImGui::GetIO(); (void)io;
...
void TrVulkanRendererRaster::CreateImGuiGraphicsPipeline()
{
...
void TrVulkanRendererRaster::CreateImGuiDescriptorSetLayout()
{
	VkDescriptorSetLayoutBinding samplerDescriptorSetLayoutBinding = {};
...
void TrVulkanRendererRaster::CreateImGuiDescriptorPool()
{
	VkDescriptorPoolSize samplerPoolSize = {};
	samplerPoolSize.type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
	samplerPoolSize.descriptorCount = static_cast<uint32_t>(TrVulkanGlobal::MAX_FRAMES_IN_FLIGHT);
...
void TrVulkanRendererRaster::CreateImGuiDescriptorSets()
{
	// layout object number should match set object number
...
void TrVulkanRendererRaster::InitImGuiBufferNum()
{
...
void TrVulkanRendererRaster::CreateTestRenderPassImGuiOnly()
{
	VkAttachmentDescription colorAttachment = {};
	colorAttachment.format = mSwapChainImageFormat;
	colorAttachment.samples = VK_SAMPLE_COUNT_1_BIT;
...
void TrVulkanRendererRaster::UpdateGlobalConfigs()
{
	// MVP Params
	// Model
...
std::vector<const char*> TrVulkanRendererRaster::GetRequiredExtensions()
{
...
bool TrVulkanRendererRaster::CheckDeviceExtensionSupport(VkPhysicalDevice device)
{
...
void TrVulkanRendererRaster::CheckExtensionSupport()
{
...
bool TrVulkanRendererRaster::CheckValidationLayerSupport()
{
...
VkBool32 TrVulkanRendererRaster::DebugCallBack(
